    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="flex min-h-screen">
      ...
      <motion.div initial={{ x: 40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="flex-1 flex items-center justify-center p-6 relative bg-[#06060f] overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[400px]">
          ...
          <h1 className="font-display text-[28px] font-bold text-white mb-2">Crie sua conta</h1>
          <p className="font-body text-[15px] text-white/50 font-light mb-8">Comece a automatizar seu atendimento hoje</p>

          <motion.div variants={container} initial="hidden" animate="visible">
            {/* OAuth */}
            <motion.div variants={fadeUp} className="grid grid-cols-2 gap-3 mb-5">
              <motion.button whileHover={{ y: -1, scale: 0.98 }} whileTap={{ scale: 0.97 }} className="flex items-center justify-center gap-2.5 py-[11px] px-4 bg-white/[0.05] border border-white/[0.1] rounded-[10px] text-sm font-medium text-white/85 hover:bg-white/[0.09] transition-colors duration-200" onClick={loginWithGoogle} aria-label="Registrar com Google">
